...
A_path = f'{path}/1-fold/A/iris'
B_path = f'{path}/1-fold/B/iris'
...
train_log_dir = 'Z:/2nd_paper/backup/Compare/Detectors/Linux/AGPAD/iDCGAN/CrossFold/2-fold/try_0/logs'
# train_log_dir = 'Z:/1st/backup/ckp/Proposed/nd/GANs/2-fold/ACL_GAN/try2/logs'
train_summary_writer = tf.summary.create_file_writer(train_log_dir)

# set CheckPoint
ckpt = tf.train.Checkpoint(step=tf.Variable(1), optimizer=optimizer, net=DenseNet_model)
checkpoint_path = 'Z:/2nd_paper/backup/Compare/Detectors/Linux/AGPAD/iDCGAN/CrossFold/2-fold/try_0/ckp'
...
def train_step(x, y):
    with tf.GradientTape() as tape:
        logits = DenseNet_model(x, training=True)
        loss_val = loss_fn(y, logits)

    grads = tape.gradient(loss_val, DenseNet_model.trainable_variables)
    optimizer.apply_gradients(zip(grads, DenseNet_model.trainable_variables))
...
for epoch in range(epochs):
    ...
    A_ds_shuffle = Mkds.configure_for_performance(A_ds, Acnt, batchsz, shuffle=True)
    ...
    for step in range(Acnt//batchsz):
        iris_img, iris_label = next(A_ds_it)
        loss_val = train_step(iris_img, iris_label)
    ...
    for i in range(Bcnt//batchsz):
        ...
        val_logits = test_step(iris_img, iris_label)
